...
class MapUtil {
   ...
   * @param {object} infoWindowRef - infoWindow的渲染函数或者单文件组件的引用
  ...
  addGeometry(data, optionCb, targetLayer, infoWindowRef) {
    ...
    const geometrys = data
      ...
      .map(([type, args, callbacks = {}]) => {
        ...
        if (infoWindowRef) {
          const infoWindowTempIns = createApp(infoWindowRef, {
            ...geometry.getProperties(),
          }).mount();
          const infoWindowIns = new this.maptalks.ui.InfoWindow({
            custom: true,
            content: infoWindowTempIns.$el,
            ...(infoWindowTempIns.geometoryOption || {}),
          }).addTo(geometry);
          infoWindowTempIns.closeInfoWindow = () => infoWindowIns.hide();
        }
